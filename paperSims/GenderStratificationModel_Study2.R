#Model function for Study 2 has been modified in order for it to accept an organizational matrix as input. 

sim <- function (externalGender=.5, hotJobEffSize=0, orgMat) {
...
initPercent.male.inp<-(sum(orgMat$Male)/nrow(orgMat)) #Percentage of initial agents that are male
...
#Turnover (TO)
##Level tenure
timeWindow <- 7 #Years at same level when time-at-level becomes influential in TO
aSlopeTime <- 2
bMidTime <- 7 #Years at same level when probability increases most rapidly
cLowerTime <- 0
